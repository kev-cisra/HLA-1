<template>
    <app-layout>
        <div class="uppercase tw-mx-4">
            ...
            <div class="tw-p-4 tw-mt-8 tw-mx-56">
                <Table id="Concentrado">
                    <template v-slot:TableHeader>
                        <th class="columna">UNIDAD</th>
                        <th class="columna">NOMBRE MATERIAL</th>
                        <th class="columna">TOTAL</th>
                    </template>

                    <template v-slot:TableFooter>
                        <tr class="fila" v-for="dato in Solicitud" :key="dato.id">
                            <td class="tw-text-center">{{ dato.Unidad }}</td>
                            <td>{{ dato.Nombre }}</td>
                            <td class="tw-text-center">{{ dato.Total }}</td>
                        </tr>
                    </template>
                </Table>
            </div>
...
export default {
    ...
    methods: {
        ...
        Filtros(){
            ...
            this.$inertia.get('/Compras/Papeleria', this.params , { //envio de variables por url
                onSuccess: () => {
                    this.loading = false;
