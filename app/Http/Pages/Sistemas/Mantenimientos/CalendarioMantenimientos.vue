    ...
    <app-layout>
        ...
        <section class="tw-flex tw-justify-between tw-content-center tw-border tw-p-1 tw-my-8 tw-mx-8">
            ...
            <div v-if="Autorizado == true">
                <jet-button @click="openReportes" class="BtnNuevo">REPORTES</jet-button>
            </div>
            <div v-if="Autorizado == true">
                            ...
                            <td>
                                <div class="FlexCenter">
                                    <div v-if="event.Estatus == 0">
                                        <span tooltip="" flow="left">
                                            <span class="tw-inline-flex tw-items-center tw-justify-center tw-h-6 tw-px-3 tw-text-white tw-bg-red-400 tw-rounded-full">FECHA SIN CONFIRMAR</span>
                                        </span>
                                    </div>
                                    <div v-if="event.Estatus == 1">
                                        <span tooltip="" flow="left">
                                            <span class="tw-inline-flex tw-items-center tw-justify-center tw-h-6 tw-px-3 tw-text-white tw-bg-blueGray-400 tw-rounded-full">FECHA CONFIRMADA</span>
                                        </span>
                                    </div>
                                    <div v-if="event.Estatus == 2">
                                        <span tooltip="" flow="left">
                                            <span class="tw-inline-flex tw-items-center tw-justify-center tw-h-6 tw-px-3 tw-text-white tw-bg-sky-400 tw-rounded-full">MANTENIMIENTO REALIZADO</span>
                                        </span>
                                    </div>
                                    <div v-if="event.Estatus == 3">
                                        <span tooltip="" flow="left">
                                            <span class="tw-inline-flex tw-items-center tw-justify-center tw-h-6 tw-px-3 tw-text-white tw-bg-teal-400 tw-rounded-full">CONFORME MANTENIMIENTO</span>
                                        </span>
                                    </div>
                                </div>
                            </td>
...
export default {
    ...
    data() {
        return {
            ...
            EventosCalendario: Object, //Objeto de historico de Mantenimeintos
            form: {
    ...
    computed:{
        ...
        BuscaDispositivo: function () {
            ...
            this.EquiposAsignados.forEach(e => {
                EquiposAsignados.push({id: e.id +'/'+e.perfil.Nombre +' '+e.perfil.ApPat+' '+e.perfil.ApMat +'-'+ e.hardware.Nombre + ' '+ e.hardware.Marca+ ' '+e.hardware.Modelo+ ' '+e.hardware.Comentarios, text: e.perfil.Nombre +' '+e.perfil.ApPat+' '+e.perfil.ApMat +'-'+ e.hardware.Nombre + ' '+ e.hardware.Marca+ ' '+e.hardware.Modelo+ ' '+e.hardware.Comentarios})
            });
